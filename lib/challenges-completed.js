...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  function updateIndex (data) {
    ...
    var counter = 0
    var completed = 0
    var locale_menu = document.getElementById("lang-select");
    var locale = locale_menu.options[locale_menu.selectedIndex].value;
    if(locale != '') {
      locale = '-' + locale;
    }
    ...
    for (var chal in data) {
      if (data[chal].completed) {
        ...
        leftOffButton.href = 'challenges' + locale + '/' + data[chal].next_challenge + '.html'
        completed++
        counter++
      } else {
        counter++
